...
Answer here:
It is String to Nat or String → Nat
-/
...
def glue_funs {x y z : Type} : (g : y → z) → (f :x → y) → (s:x) → z
| g, f, s=> g (f s)
-- test cases
#eval glue_funs isEven String.length "Hello"  -- false
#eval glue_funs isEven String.length "Hello!" -- true
...
No. It is applying the second function to the number then the first to the result of the second.
because its square and double, it changes your answer depending on the order. 
-/
