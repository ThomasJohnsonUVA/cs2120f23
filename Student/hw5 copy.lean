...
def not_either_not_both { jam cheese } :
  ((no jam) ⊕ (no cheese)) → 
  (no (jam × cheese)) 
| Sum.inl nojam => (fun jc => (nojam jc.1))
| Sum.inr nocheese => (fun jc => (nocheese jc.2))
...
| (Sum.inl noa) => (fun ab => noa ab.1)
| (Sum.inr nob) => (fun ab => nob ab.2)
...
fun noaorb => (fun noa => noaorb (Sum.inl noa), fun nob => noaorb (Sum.inr nob))
...
fun noab => match noab with
| (noa,nob) => fun ab => match ab with
| Sum.inl a => noa a
| Sum.inr b => nob b
...
#eval sum_f (fun x => x) 5
#eval sum_f (fun x => x + 1) 5
#eval sum_f (fun x => x * x) 3



/-
I used chatgpt for this assignment. It couldn't do any of the logic but it was actually helpful in that I kept on asking it to explain 
things about recursion, shells and solid, etc. It was good at giving the logic in english, not in lean. 
